...
if(RUN$qualitative_analysis) {
  ...
  for (i in 1:n_reps) {
    ...
    model_fPCA_gcv_load_HR <- cbind(
      evaluate_field(grid, model_fPCA_gcv$results$loadings[,1], mesh),
      evaluate_field(grid, model_fPCA_gcv$results$loadings[,2], mesh),
    ...
    adjusted_results_fPCA_gcv <- adjust_results(
      ...
      loadings_true_locs,
      list(
        loadings = model_fPCA_gcv$results$loadings,
        loadings_HR = model_fPCA_gcv_load_HR
      ))
    
    ## ajust loadings model_fPCA_mon_hyb
    model_fPCA_mon_hyb_load_HR <- cbind(
      evaluate_field(grid, model_fPCA_mon_hyb$results$loadings[,1], mesh),
      evaluate_field(grid, model_fPCA_mon_hyb$results$loadings[,2], mesh),
      evaluate_field(grid, model_fPCA_mon_hyb$results$loadings[,3], mesh)
    )
    adjusted_results_fPCA_mon_hyb <- adjust_results(
    ...
    scores$fPCA_gcv[[i]] <- adjusted_results_fPCA_gcv$scores
