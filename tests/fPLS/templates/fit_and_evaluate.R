...
if (file.exists(file_model) && !FORCE_FIT) {
  ...
  ## model fit
  start.time <- Sys.time()
  model_MV_PLS <- MV_PLS_wrapped(data, n_comp = n_comp, center = mean, mode = mode_MV)
  end.time <- Sys.time()
...
if(!is.null(model_MV_PLS)) {
  ...
  ## X_mean
  if(!mean) {
    model_MV_PLS$results$Y_mean <- NULL
    model_MV_PLS$results$X_mean_locs <- NULL
  }
  
  ## model evaluation
  results_evaluation$MV_PLS <- evaluate_results(model_MV_PLS, generated_data)
...
if(!is.null(model_fPLS_off)) {
  ...
  ## model evaluation
  results_evaluation$fPLS_off <- evaluate_results(model_fPLS_off, generated_data)
...
if(!is.null(model_fPLS_gcv)) {
  ...
  ## model evaluation
  results_evaluation$fPLS_gcv <- evaluate_results(model_fPLS_gcv, generated_data)
